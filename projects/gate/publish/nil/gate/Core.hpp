    ...
    public:
            ...
            node(typename detail::traits<T>::i::readonly_edges input_edges, Args&&... args)
        {
            owned_nodes.emplace_back(
                std::make_unique<detail::Node<T>>(input_edges, std::forward<Args>(args)...)
            );
            return static_cast<detail::Node<T>*>(owned_nodes.back().get())->output_edges();
        }
        ...
        {
            required_edges.emplace_back(std::make_unique<MutableEdge<T>>());
            return static_cast<MutableEdge<T>*>(required_edges.back().get());
        }
